...
manager_kwargs = {
    ...
    'output_op': lambda: tf.keras.layers.Lambda(lambda x: tf.math.log(x)/np.log(10), name="output_log"),
...
def plot_dataframe(data):
    ...
    ax = axs[1]
    sns.scatterplot(x='kinn', y=np.log10(plot_df.Read+1),
                    hue='sgRNA_type', alpha=0.5, data=plot_df, ax=ax)
    ...
    ax = axs[0]
    sns.lineplot(x=q, y=np.percentile(plot_df.kinn, q),
...
def predict_on_dataframe(data, is_aligned=True):
    ...
    # load cnn
    wd = config['dcnn_1']
    train_hist = pd.read_table(os.path.join(
        wd, "train_history.csv"), sep=",", header=None)
    best_trial_id = train_hist.sort_values(2, ascending=False).head(1)[0]
    dcnn_1 = tf.keras.models.load_model(os.path.join(
        wd, "weights", "trial_%i" % best_trial_id, "bestmodel.h5"))

    wd = config['dcnn_2']
    train_hist = pd.read_table(os.path.join(
        wd, "train_history.csv"), sep=",", header=None)
    best_trial_id = train_hist.sort_values(2, ascending=False).head(1)[0]
    dcnn_2 = tf.keras.models.load_model(os.path.join(
        wd, "weights", "trial_%i" % best_trial_id, "bestmodel.h5"))
