...
from src.model_spaces import get_sim_model_space 
...
def get_data(filepath:str, logbase:float=10, noise_sigma:float=0):
    """Get sequences and kinetic rates to from simulated data files to train KINNs. 
    Applies noise to data if specified

    Parameters
    ----------
    filepath : str
        Path to data file. Must either be .csv or .tsv
    logbase : float, optional
        Logarithmic base applied to kinetic rates, by default 10
    noise_sigma : float, optional
        Noise applied to training and validation set, by default 0

    Returns
    -------
    _type_
        Returns training input sequences and output rates,
        test sequences and output rates

    Raises
    ------
    Exception
        File path not found if not .tsv or .csv
    """
    if filepath.endswith("csv"):
        input_seqs_ohe, y_train, test_seqs_ohe, y_test = load_csv(filepath)
    elif filepath.endswith("tsv"):
        input_seqs_ohe, y_train, test_seqs_ohe, y_test = load_tsv(filepath)
    else:
        raise Exception("filepath not understood: %s" % filepath)
...
def load_csv(filepath:str, output_act=True):
    """Given a simulated data file path extract input sequences and 
    associated kinetic rates spliting the data into training and test sets

    Parameters
    ----------
    filepath : str
        Path to data
    output_act : bool, optional
    ...
    -------
    _type_
        Training input sequences, training output rates,
        test sequences, test output rates

    """
    # Create one-hot encoder for input sequence
    x_set = [['A'], ['C'], ['G'], ['T']]
...
def main():
    args = parse()
    kinn_model_space = get_sim_model_space(use_sink_state=args.use_sink_state)
    ...
    neural_search_controller = pmbga.ProbaModelBuildGeneticAlgo(
                ...
                buffer_size=50,  # buffer size controls the max history going back
    ...
    res = get_data(filepath=args.data_file, 
                   logbase=args.logbase, 
                   noise_sigma=args.noise_sigma)
    (x_train, y_train), (x_test, y_test) = res
    
    logbase = args.logbase
    # Create an AMBER operation for converting activity to phenotype
    if args.use_sink_state:
        output_operation = lambda: tf.keras.layers.Lambda(lambda x: 
            tf.math.log(tf.clip_by_value(tf.reshape(-x[:,1], (-1,1)), 10**-16, 10**3))/np.log(logbase), 
                                                   name="output_slice")
    else:
        output_operation = lambda: tf.keras.layers.Lambda(lambda x: 
            tf.math.log(tf.clip_by_value(x, 10**-16, 10**3))/np.log(logbase), 
                                                   name="output_log")

    # trainEnv parameters (What are trainEnv parameters?
    evo_params = dict(
    ...
    manager_kwargs = {
        'output_op': output_operation,
        'n_feats': 50,
        'n_channels': 4,
        'batch_size': batch_size,
        'epochs': 20 if args.use_sink_state else 100,
        'earlystop': 5,
        'optimizer': lambda: tf.keras.optimizers.Adam(learning_rate=learn_rate_schedule, clipnorm=1.0),
        'verbose': 0
    }

    # Search neural architecture
    neural_search_controller, hist, stat_df = search_env(
        neural_search_controller=neural_search_controller,
    ...
    # Once run is completed, plot the best model against ground truth statistics
    model_builder = reload_from_dir(workdir=args.workdir,
                         manager_kwargs=manager_kwargs, 
                         model_fn=evo_params['model_fn'])
    
    tf.keras.utils.plot_model(model_builder.model, to_file=os.path.join(args.workdir, "model.png"))
    y_hat = model_builder.predict(x_test).flatten()
    ...
    plot_model_post_distr(neural_search_controller=neural_search_controller, args=args)
    return neural_search_controller


def plot_model_post_distr(neural_search_controller, args):
   ...
   # START SITE
    ...
    for k in neural_search_controller.model_space_probs:
        if k[-1] == 'RANGE_ST':
            try:
                d = neural_search_controller.model_space_probs[k].sample(size=1000)
            except:
                continue
            ax = axs[k[0]]
            _ = sns.distplot(d, label="Post", ax=ax)
            _ = sns.distplot(neural_search_controller.model_space_probs[k].prior_dist, 
                             label="Prior", ax=ax)
            if k[0] < 7:
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(args.workdir, "syn_range_st.png"))
    ...
    for k in neural_search_controller.model_space_probs:
        if k[-1] == 'RANGE_D':
            ...
            _ = sns.distplot(neural_search_controller.model_space_probs[k].prior_dist, label="Prior", ax=ax)
            if k[0] < 7:
            ...
            #_ = ax.set_xlim(0,20)    
    fig.tight_layout()
    fig.savefig(os.path.join(args.workdir, "syn_range_d.png"))
    ...
    for k in neural_search_controller.model_space_probs:
        if k[-1] == 'EDGE':
            ...
            sns.distplot(neural_search_controller.model_space_probs[k].prior_dist, ax=ax)
            ax.set_title(
                ' '.join(['Rate ID', str(k[0]), '\nPosterior mean', str(np.mean(d))]))
            #_ = ax.set_xlim(0,20)    
    fig.tight_layout()
    fig.savefig(os.path.join(args.workdir, "syn_edge.png"))
