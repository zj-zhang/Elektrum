from src.kinetic_model import KineticModel, KingAltmanKineticModel, modelSpace_to_modelParams
...
def search_env(neural_search_controller:pmbga.ProbaModelBuildGeneticAlgo, 
               workdir:str, 
        ...
        try:
            ...
            for _ in range(samps_per_gen):
                ...
                if test_reward > best_indv:
                    ...
                    has_impr = True
                    shutil.move(os.path.join(workdir, "bestmodel.h5"), os.path.join(workdir, "AmberSearchBestModel.h5"))
                    shutil.move(os.path.join(workdir, "model_params.pkl"), os.path.join(workdir, "AmberSearchBestModel_config.pkl"))
                # store
            ...
            if disable_posterior_update is True:
                neural_search_controller.buffer.finish_path(neural_search_controller.model_space, generation, working_dir=workdir)
            else:
                _ = neural_search_controller.train(episode=generation, working_dir=workdir)
            post_vars = [np.var(x.sample(size=100)) for _, x in neural_search_controller.model_space_probs.items()]
    ...
    # save
    pickle.dump(neural_search_controller, open(os.path.join(workdir, "controller_states.pkl"), "wb"))

    # plot
    make_plots(neural_search_controller, canvas_nrow=np.ceil(np.sqrt(len(neural_search_controller.model_space))), wd=workdir)
...
def get_reward_pipeline(model_arcs, 
    ...
    with train_graph.as_default(), train_sess.as_default():
        ...
        checkpointer = ModelCheckpoint(filepath=os.path.join(workdir,"bestmodel.h5"), mode='min', verbose=0, save_best_only=True,
